...
```sh
docker run --rm -v $PWD:/tmp degauss/st_census_tract:0.0.1 my_address_file_geocoded.csv
```

will produce `my_address_file_geocoded_st_census_tract_v0.0.1.csv` with an added columns named `census_tract_vintage` and `census_tract_id`. 
